...
mnist_train <-  read.csv(paste0(dir, 'mnist_train.csv'))
...
matrice_couts <- function(n) {
  ...
  matrice_gaussienne <- matrix(rnorm(n * n), nrow = n, ncol = n)
...
liste_dimension <- c(20,50,100,500,1000)

#temps_exe <- log(temps_execution(liste_dimension,5,algo_naif)) #mettre l'algo naif dedans

#plot(log(liste_dimension),temps_exe,xlab='log(nombre de classes)',ylab='log(temps')

#reg <- lm(temps_exe~log(liste_dimension))
...
<p>
Ce processus garantit de trouver l'assignation optimale avec le coût minimal, mais avec une complexité de O(n!), ce qui peut être inefficace pour des grandes valeurs de n.
</p>
